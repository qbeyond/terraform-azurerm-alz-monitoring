...
resource "azurerm_role_definition" "function_blob_access" {
  name        = "Blob Reader and Writer"
  scope       = azurerm_storage_account.sa_func_app[0].id
  description = "Custom role to read and write blobs without delete permissions."
  ...
  assignable_scopes = [
    azurerm_storage_account.sa_func_app[0].id
  ]
}
