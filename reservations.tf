...
resource "azurerm_automation_schedule" "once_monthly" {
  ...
  start_time = time_static.monthly_start.rfc3339
}

resource "azurerm_automation_job_schedule" "reservation_to_law" {
  resource_group_name     = var.automation_account.resource_group_name
  automation_account_name = var.automation_account.name
  schedule_name           = azurerm_automation_schedule.once_monthly.name
  runbook_name            = azurerm_automation_runbook.reservation_to_law.name
  ...
  lifecycle {
    replace_triggered_by = [azurerm_automation_runbook.reservation_to_law]
  }
}
